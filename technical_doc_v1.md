...
- Provider abstraction (using an OpenAI-compatible client instead of a vendor-specific SDK) keeps the LLM provider swappable with a config change, not a code change.
- Real production friction — dynamic port binding, environment configuration — is where a lot of the actual learning happens, alongside the AI integration itself.
